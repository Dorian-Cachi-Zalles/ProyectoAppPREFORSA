...
class DatosPROCESIPSProvider with ChangeNotifier {
  ...
  List<DatosPROCESIPS> get datosprocesipsList =>
      List.unmodifiable(_datosprocesipsList);
  ...
  Future<void> _loadData() async {
    ...
    _datosprocesipsList =
        maps.map((map) => DatosPROCESIPS.fromMap(map)).toList();
    notifyListeners();
  }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          action: SnackBarAction(
            ...
            onPressed: () async {
              final newId =
                  await _db.insert(tableDatosPROCESIPS, deletedData.toMap());
              _datosprocesipsList.insert(
                  index, deletedData.copyWith(id: newId));
              notifyListeners();
            },
          ),
        ),
...
class ScreenListDatosPROCESIPS extends StatelessWidget {
  const ScreenListDatosPROCESIPS({super.key});
            ...
            return ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                return SwipeableTile.card(
                  ...
                  onSwiped: (_) =>
                      provider.removeDatito(context, dtdatosprocesips.id!),
                  ...
                  child: GestureDetector(
                    ...
                    child: GradientExpandableCard(
                      ...
                      expandedContent: [
                        ExpandableContent(
                            label: 'Hora: ',
                            stringValue: dtdatosprocesips.Hora.toString()),
                        ExpandableContent(
                            label: 'PAProd: ',
                            stringValue: dtdatosprocesips.PAProd.toString()),
                        ExpandableContent(
                            label: 'TempTolvaSec: ',
                            doubleListValue: dtdatosprocesips.TempTolvaSec),
                        ExpandableContent(
                            label: 'TempProd: ',
                            stringValue: dtdatosprocesips.TempProd.toString()),
                        ExpandableContent(
                            label: 'Tciclo: ',
                            stringValue: dtdatosprocesips.Tciclo.toString()),
                        ExpandableContent(
                            label: 'Tenfri: ',
                            stringValue: dtdatosprocesips.Tenfri.toString()),
                      ],
...
class EditDatosPROCESIPSForm extends StatefulWidget {
  ...
  const EditDatosPROCESIPSForm(
      {required this.id, required this.datosPROCESFIPS, super.key});
                  ...
                  child: ElevatedButton.icon(
                    ...
                    onPressed: () {
                      ...
                      final updatedDatito = widget.datosPROCESFIPS.copyWith(
                        ...
                        PAProd:
                            values['PAProd'] ?? widget.datosPROCESFIPS.PAProd,
                        TempTolvaSec: values['TempTolvaSec'] ??
                            widget.datosPROCESFIPS.TempTolvaSec,
                        TempProd: (values['TempProd']?.isEmpty ?? true)
                            ? 0
                            : double.tryParse(values['TempProd']),
                        Tciclo: (values['Tciclo']?.isEmpty ?? true)
                            ? 0
                            : double.tryParse(values['Tciclo']),
                        Tenfri: (values['Tenfri']?.isEmpty ?? true)
                            ? 0
                            : double.tryParse(values['Tenfri']),
                      );

                      Provider.of<DatosPROCESIPSProvider>(context,
                              listen: false)
          ...
          ...List.generate(
            ...
            (index) => Padding(
              ...
              child: FormBuilderTextField(
                ...
                initialValue:
                    widget.datosPROCESFIPS.TempTolvaSec[index].toString(),
                onChanged: (value) {
                  final field =
                      _formKey.currentState?.fields['TempTolvaSec_$index'];
                  field?.validate(); // Valida solo este campo
                  field?.save();
                },
                autovalidateMode: AutovalidateMode.onUserInteraction,
                decoration: InputDecoration(
                  labelText: 'TempTolvaSec ${index + 1}',
                  labelStyle: const TextStyle(
                      fontSize: 20,
                      color: Color.fromARGB(255, 20, 100, 96),
                      fontWeight: FontWeight.bold),
                  ...
                  errorStyle: const TextStyle(
                    fontSize: 13, // Tamaño de fuente del mensaje de error
                    height:
                        1, // Altura de línea (mayor para permitir dos líneas)
                    color: Colors
                        .red, // Color del mensaje de error (puedes personalizarlo)
                  ),
                  border: OutlineInputBorder(
                  ...
                  suffixIcon: Builder(
                    builder: (context) {
                      final isValid = _formKey.currentState
                              ?.fields['TempTolvaSec_$index']?.isValid ??
                          false;
                      return Icon(
                        isValid ? Icons.check_circle : Icons.error,
                        color: isValid ? Colors.green : Colors.red,
                ...
                validator: FormBuilderValidators.compose([
                  ...
                  FormBuilderValidators.integer(
                      errorText: 'Debe ser un número 7f entero'),
                  FormBuilderValidators.min(0,
                      errorText: 'Debe ser mayor o 7f igual a 0'),
                  FormBuilderValidators.max(100,
                      errorText: 'Debe ser menor o 7f igual a 100'),
                ]),
              ),
            ),
          ),
          ...
          FormBuilderTextField(
            ...
            decoration: InputDecoration(
              ...
              suffixIcon: Builder(
                builder: (context) {
                  final isValid =
                      _formKey.currentState?.fields['TempProd']?.isValid ??
                          false;
                  return Icon(
                    isValid ? Icons.check_circle : Icons.error,
                    color: isValid ? Colors.green : Colors.red,
          ...
          FormBuilderTextField(
            ...
            decoration: InputDecoration(
              ...
              suffixIcon: Builder(
                builder: (context) {
                  final isValid =
                      _formKey.currentState?.fields['Tciclo']?.isValid ?? false;
                  return Icon(
                    isValid ? Icons.check_circle : Icons.error,
                    color: isValid ? Colors.green : Colors.red,
                  );
          ...
          FormBuilderTextField(
            ...
            decoration: InputDecoration(
              ...
              suffixIcon: Builder(
                builder: (context) {
                  final isValid =
                      _formKey.currentState?.fields['Tenfri']?.isValid ?? false;
                  return Icon(
                    isValid ? Icons.check_circle : Icons.error,
                    color: isValid ? Colors.green : Colors.red,
                  );
